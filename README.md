...
- В репозитории нет рабочих API-ключей, паролей базы данных и адресов частной
  инфраструктуры.
- Загруженные MIDI и MP3 обрабатываются во временном каталоге и удаляются после
  ответа. Приложение не сохраняет их в репозитории или базе данных.
- Backend возвращает клиенту общие сообщения об ошибках и не раскрывает ответы
  внешних API.
- CORS ограничен списком источников из `CORS_ORIGINS`.
- Порт MySQL доступен только сервисам внутри Docker Compose.

Перед публичной публикацией проверьте не только текущие файлы, но и историю Git.
Если пользовательские файлы или секреты когда-либо попадали в коммиты, удалите
их из истории и отзовите соответствующие ключи.
